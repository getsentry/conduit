    ...
    for old_stream in old_streams {
        ...
        match redis.delete_stream(&old_stream).await {
            ...
            Err(e) => {
                ...
                metrics::counter!("delete.failed").increment(1);
...
fn main() -> anyhow::Result<()> {
    ...
    let logging_config = logging::LoggingConfig::new(
        std::env::var("CLEANUP_SENTRY_DSN").ok(),
...
async fn async_main() -> anyhow::Result<(), anyhow::Error> {
    let port = std::env::var("CLEANUP_PORT")?;
    let redis_url = std::env::var("REDIS_URL")?;

    let worker_interval_sec = env::var("CLEANUP_INTERVAL_SEC")
        .ok()
                ...
                _ = interval.tick() => {
                    ...
                    let (total, deleted, untracked) =
                        match cleanup_old_stream(&broker, cutoff_timestamp).await {
                            Ok(counts) => {
                                metrics::counter!("cycles.completed").increment(1);
                                counts
                            }
                            Err(e) => {
                                tracing::error!(error = %e, "Failed to get old streams for cleanup");
                                metrics::counter!("cycles.failed").increment(1);
                                continue;
                            }
                        };
                    metrics::counter!("streams.found").increment(total as u64);
                    metrics::counter!("streams.deleted").increment(deleted as u64);
                    metrics::counter!("streams.untracked").increment(untracked as u64);

                    let duration_ms = start.elapsed().as_millis() as f64;
                    metrics::histogram!("duration_ms").record(duration_ms);
